...
from transformers.generation.logits_process import NoRepeatNGramLogitsProcessor, RepetitionPenaltyLogitsProcessor, \
    MinLengthLogitsProcessor, TemperatureLogitsWarper, TopKLogitsWarper, TopPLogitsWarper, NoBadWordsLogitsProcessor
import torch
import clip
...
class CLIPTextGenerator:
    ...
    def __init__(self,
        ...
        # Initialize dummy context as parameters for optimization
        self.dummy_context_offset = [(torch.nn.Parameter(torch.zeros(k.shape, device=self.device).type_as(k)),
                                      torch.nn.Parameter(torch.zeros(v.shape, device=self.device).type_as(v)))
        ...
        self.prevent_early_finish = MyMinLengthLogitsProcessor(target_seq_length // 2, self.end_token)
    ...
    def generate(self, image_features: torch.Tensor):
        ...
        for gd_iter in range(self.sentence_iterations):
            ...
            for i in range(self.target_seq_length):
                ...
                # Ensure these tensors have requires_grad=True
                clip_loss, _ = self.clip_loss(probs, generated_tokens, 512)
                clip_losses.append(clip_loss.item())
                
                # Create a version of unshifted_probs that has requires_grad if needed
                if not unshifted_probs.requires_grad and probs.requires_grad:
                    unshifted_probs_with_grad = unshifted_probs.detach().clone().requires_grad_(True)
                    fluency_loss = -(unshifted_probs_with_grad * probs.log()).sum()
                else:
        ...
        # Process final results with no_grad to avoid unnecessary gradient tracking
        with torch.no_grad():
    ...
    def decode_beam_search(self, shifted_context, beam_size, prefix_text='Image of a'):
        # Đảm bảo shifted_context ở đúng device
        shifted_context = [(k.to(self.device), v.to(self.device)) for k, v in shifted_context]
        ...
        for i in range(self.target_seq_length):
            ...
            if scores is None:
                ...
                gen_tokens = next_tokens.to(self.device)
            else:
                ...
                next_tokens = next_tokens.unsqueeze(1).to(self.device)
    ...
    def update_special_tokens_logits(self, context_tokens, i, logits):
        ...
        try:
            # Deter tokens that appeared in similar form in recent history
            history_len = min(16, context_tokens.shape[1])  # Đảm bảo không vượt quá độ dài context
            history_tokens = context_tokens[:, -history_len:]
            
            # Đảm bảo history_tokens và token_to_similar_indices ở cùng device
            if history_tokens.device != self.token_to_similar_indices.device:
                print(f"Warning: history_tokens device {history_tokens.device} khác với token_to_similar_indices device {self.token_to_similar_indices.device}")
                history_tokens = history_tokens.to(self.token_to_similar_indices.device)
            ...
            print(f"Bỏ qua deter_small_repeat vì lỗi: {str(e)}")
            
        try:
            # Prevent exact duplicate n-grams from anywhere in context
            logits = self.prevent_large_repeat(context_tokens, logits)
        except Exception as e:
            print(f"Bỏ qua prevent_large_repeat vì lỗi: {str(e)}")

        try:
            # Prevent premature ending
            logits = self.prevent_early_finish(context_tokens, logits)
        except Exception as e:
            print(f"Bỏ qua prevent_early_finish vì lỗi: {str(e)}")

        try:
            # Prevent generation of tokens from the forbidden token list
            logits = self.prevent_forbidden_tokens(context_tokens, logits)
        except Exception as e:
            print(f"Bỏ qua prevent_forbidden_tokens vì lỗi: {str(e)}")

        # Reduce probability of unwanted styled words
        token_offset = self.first_token_offset if i == 0 else self.other_token_offset
        token_offset = token_offset.to(logits.device)
        logits += token_offset.unsqueeze(0)

        # Give a tiny constant nudge towards ending
        ...
        if i >= self.target_seq_length - self.prevent_early_finish.min_length:
            ...
            logits[logits < top_threshold_values[:, -1:]] = -float('inf')
            logits[:, self.end_token] = ending_threshold
            required_bump = top_threshold_values[:, -1] > ending_threshold
            logits[required_bump, top_threshold_indices[required_bump, -1]] = -float('inf')
    ...
    def text_to_video_similarity(self, text_cont: List[str]):
        with torch.no_grad():
            ...
            # Đảm bảo text_features và image_features ở cùng device
            text_features = text_features.to(self.device)
            
        # Make a copy of image_features to ensure it's tracked for gradients
        image_features = self.image_features.to(self.device)
        
        # Properly track for gradients outside the no_grad context
        similarity = self.logit_scale * (text_features @ image_features.T)
        
        # Ensure correct dtype while preserving gradients
        if similarity.dtype != torch.float32:
    ...
    def clip_loss(self, probs, context_tokens, samples: int):
        ...
        top_next_tokens = top_next_tokens.to(self.device)
        ...
        for prefix_text, prefix_top_next_tokens, prefix_next_token_prob in zip(prefix_texts, top_next_tokens, probs):
            ...
            # Take the clip probabilities as ground truth prior over the continuations
            next_token_probs = prefix_next_token_prob[prefix_top_next_tokens].to(self.device)
            # Ensure tensors have requires_grad=True to maintain gradient information
            if not next_token_probs.requires_grad:
                next_token_probs = next_token_probs.detach().clone().requires_grad_(True)
                
            cont_video_loss = -cont_video_prob * next_token_probs.log().unsqueeze(-1)
            prefix_losses.append(cont_video_loss.sum(0).mean())

        # Ensure the final tensor maintains gradient information
        prefix_losses_tensor = torch.stack(prefix_losses).to(self.device)
        total_loss = torch.sum(prefix_losses_tensor)
        return total_loss, prefix_losses

class MyMinLengthLogitsProcessor(MinLengthLogitsProcessor):
    """
    Phiên bản tùy chỉnh của MinLengthLogitsProcessor để đảm bảo tất cả tensor đều ở cùng device
    """
    def __call__(self, input_ids, scores):
        ...
        if cur_len < self.min_length:
            ...
            if hasattr(self, "eos_token_id"):
                ...
                scores = scores.index_fill(dim=-1, index=eos_token_id, value=-float("inf"))
            else:
                # Nếu không có eos_token_id, ta sử dụng thủ công
