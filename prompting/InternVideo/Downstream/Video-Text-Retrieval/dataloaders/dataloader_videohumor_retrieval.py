...
class VideoHumor_DataLoader(Dataset):
    ...
    def _get_text(self, sentences_set, starts=None, ends=None, t_stride=None):
        ...
        for s, e in zip(starts, ends):
            ...
            for i in range(s, e, t_stride):
                index = int(float(i)/t_stride)
                if 0 <= index < len(sentences_set):
                    sentences.extend(sentences_set[index])
                else:
                    print(f"Warning: Index {index} out of range for sentences_set with length {len(sentences_set)}")
    ...
    def _get_rawvideo_dec(self, vreader, starts=None, ends=None, t_stride=None):
        ...
        video_mask = np.zeros((len(starts), self.max_frames), dtype=np.int64)
        ...
        video = np.zeros((len(starts), self.max_frames, 1, 3,
                          self.image_resolution, self.image_resolution), dtype=np.float64)
        ...
        for i, (f_start, f_end) in enumerate(zip(starts, ends)):
            ...
            if f_end >= f_start:
                ...
                # Lấy kích thước video từ trước để dùng khi cần tạo frame trống
                try:
                    # Thử lấy frame đầu tiên để biết kích thước
                    frame_shape = None
                    try:
                        if len(vreader) > 0:
                            # Lấy kích thước frame từ tham số get_shape của decord
                            frame_shape = vreader.get_shape()
                            frame_height, frame_width = frame_shape[0], frame_shape[1]
                    except:
                        # Nếu không lấy được kích thước, dùng giá trị mặc định
                        frame_height, frame_width = 360, 640
                        
                    # Thêm xử lý lỗi khi đọc batch frames
                    try:
                        # Thử đọc toàn bộ batch
                        batch_frames = vreader.get_batch(sample_pos).numpy()
                        patch_images = [Image.fromarray(f) for f in batch_frames]
                    except Exception as e:
                        print(f"Lỗi khi đọc batch frames: {str(e)[:100]}...")
                        # Thử đọc từng frame một khi batch bị lỗi
                        patch_images = []
                        for pos in sample_pos:
                            try:
                                # Thử đọc frame đơn lẻ
                                frame = vreader[pos].numpy()
                                patch_images.append(Image.fromarray(frame))
                                # Cập nhật kích thước frame nếu chưa có
                                if frame_shape is None:
                                    frame_height, frame_width = frame.shape[0], frame.shape[1]
                            except Exception as frame_e:
                                print(f"Không thể đọc frame {pos}: {str(frame_e)[:100]}...")
                                # Nếu frame bị lỗi, tạo frame trống để duy trì cấu trúc
                                empty_frame = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
                                patch_images.append(Image.fromarray(empty_frame))
                    
                    if not patch_images:
                        print(f"Không thể đọc bất kỳ frame nào cho đoạn {i}")
                        continue
                    ...
                    print(f"Lỗi khi xử lý patch images: {e}")
            else:
    ...
    def __getitem__(self, idx):
        ...
        dur = librosa.get_duration(path=f'{vdir}/audio.wav')
