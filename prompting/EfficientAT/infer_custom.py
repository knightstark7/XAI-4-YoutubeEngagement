...
def audio_tagging(root_dir, audtag_filename): 
    ...
    # Get all audio files from root_dir directory
    audios = glob(f'{root_dir}/*/audio.wav')
    
    print(f"Tìm thấy {len(audios)} file audio để xử lý")
    
    for audio in audios:
        try:
            print(f"Đang xử lý file audio: {audio}")
            result = run(model, mel, sample_rate, audio, device)
            # Save result in the same directory as audio file
            save_path = os.path.join(os.path.dirname(audio), audtag_filename)
            torch.save(result, save_path)
            print(f"Đã lưu kết quả vào: {save_path}")
        except Exception as e:
            print(f"Lỗi khi xử lý file {audio}: {str(e)}")
            # Vẫn tạo file audtag trống để tránh lỗi
            try:
                empty_result = []
                save_path = os.path.join(os.path.dirname(audio), audtag_filename)
                torch.save(empty_result, save_path)
                print(f"Đã tạo file audtag trống: {save_path}")
            except Exception as err:
                print(f"Không thể tạo file audtag trống: {str(err)}")


if __name__ == "__main__":
    root_dir = "pipeline/videos"  # Set default root directory
    audtag_filename = "audtag.pt"  # Set default output filename
    audio_tagging(root_dir, audtag_filename)